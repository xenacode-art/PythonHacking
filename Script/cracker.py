...
#by default, Kali linux uses sha512 encryption and goes 5000 rounds and also uses salt to 
defeat rainbow attack 
...
comparisms 
 ...
 if hashes[i] == sha512_crypt.using(rounds=5000, 
salt=salts[i]).hash(newpass).split("$")[-1]: 
